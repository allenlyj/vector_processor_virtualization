...
static void vp_tlb(vp_control *vp)
{
	...
	printf("Vector Length:    %8d\t %8d\t %8d\t %8d\t\n",vp->thread_len[0],
		vp->thread_len[1],vp->thread_len[2],vp->thread_len[3]);
	printf("Num of Registers: %8d\t %8d\t %8d\t %8d\t\n",vp->thread_num[0],
		vp->thread_num[1],vp->thread_num[2],vp->thread_num[3]);
...
static int process_id (int reg_len, int reg_num, vp_control *vp)
{
	...
	if (pid>3)
	{
#if !defined(__RUN_ON_MB_) && defined(__MISS_MSG_)
...
static void access_reg_64 (int num,int pid,vp_control *vp)
{
	...
	for (int i=0; i<num; i++)
	{
		...
		// Remove register from access que and label as used
		vp->head_64 = current->next;
		current->used = 1;
		current->in_que = 0;
...
#ifdef __RUN_ON_MB_
	// This is Xilinx MB specific function used to interface the hardware TLB (custom design)
	int tlb_signal = entry | (pid<<5) | (rname<<7);
	putfslx(tlb_signal,0,FLS_DEFAULT);
	//printf("An MB stream instruction should be sent here!\n");
#endif
...
static void recover_64 (int rname, vp_control *vp)
{
	...
	vp->reg_64[rname].next = vp->head_64;
...
void thread_release (int pid, vp_control *vp)
{
	...
	for (int i=0; i<vp->thread_num[pid]; i++)
		{
